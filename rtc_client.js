var MyRTC = function() {
    ...
    var config = {
        "iceServers": [{
            ...
            // "url": "stun:stun2.1.google.com:1930/2"
        }]
    };
    var packetSize = 1000;
    ...
    function RTC() {
        ...
        this.receiveFiles = {};
        //保存当前用户名字
        this.username=window.location.href.split("/").reverse()[0];
    }
    ...
    //创建单个PeerConnection
    RTC.prototype.createPeerConnection = function(socketId) {
        var that = this;
        var pc = new PeerConnection(config);//局域网内通信
    ...
    //关闭PeerConnection连接
    RTC.prototype.closePeerConnection = function(pc) {
        if (!pc) return;
        pc.close();
    };
    ...
    //消息广播
    RTC.prototype.broadcast = function(message) {
        console.log(message);
    ...
    //发送消息方法
    RTC.prototype.sendMessage = function(message, socketId) {
    ...
    RTC.prototype.createDataChannel = function(socketId, label) {
        var pc, key, channel;
    ...
    RTC.prototype.addDataChannel = function(socketId, channel) {
        var that = this;
        ...
        channel.onmessage = function(message) {
            ...
            if (json.type === '__file') {
                ...
                that.emit('data_channel_message', channel,message.user, json);
            }
        };
    ...
    RTC.prototype.parseFilePacket = function(json, socketId) {
    ...
    RTC.prototype.shareFile = function(dom) {
        var socketId,
            that = this;
    ...
    //向某一单个用户发送文件
    RTC.prototype.sendFile = function(dom, socketId) {
        var that = this,
            file,
            reader,
            fileToSend,
    ...
    //发送多个文件的碎片
    RTC.prototype.sendFileChunks = function() {
        var socketId,
            sendId,
            that = this,
            nextTick = false;
    ...
    //发送某个文件的碎片
    RTC.prototype.sendFileChunk = function(socketId, sendId) {
        var that = this,
    ...
    //发送文件请求后若对方同意接受,开始传输
    RTC.prototype.receiveFileAccept = function(sendId, socketId) {
        var that = this,
            fileToSend,
            reader,
    ...
    RTC.prototype.receiveFileRefuse = function(sendId, socketId) {
        var that = this;
    ...
    //清除发送文件缓存
    RTC.prototype.cleanSendFile = function(sendId, socketId) {
        var that = this;
        delete that.fileChannels[socketId][sendId];
    };

    //发送文件请求
    RTC.prototype.sendAsk = function(socketId, sendId, fileToSend) {
        var that = this,
            channel = that.dataChannels[socketId],
            packet;
        if (!channel) {
    ...
    //获得随机字符串来生成文件发送ID
    RTC.prototype.getRandomString = function() {
    ...
    //接收到文件碎片
    RTC.prototype.receiveFileChunk = function(data, sendId, socketId, last, percent) {
        var that = this,
    ...
    RTC.prototype.getTransferedFile = function(sendId) {
        var that = this,
            fileInfo = that.receiveFiles[sendId],
    ...
    //接收到发送文件请求后记录文件信息
    RTC.prototype.receiveFileAsk = function(sendId, fileName, fileSize, socketId) {
        var that = this;
        ...
        console.log("receive file ask");
        that.emit("receive_file_ask", sendId, socketId, fileName, fileSize);
    };

    //发送同意接收文件信令
    RTC.prototype.sendFileAccept = function(sendId) {
        var that = this,
            fileInfo = that.receiveFiles[sendId],
    ...
    //发送拒绝接受文件信令
    RTC.prototype.sendFileRefuse = function(sendId) {
        var that = this,
            fileInfo = that.receiveFiles[sendId],
            channel = that.dataChannels[fileInfo.socketId],
            packet;
    ...
    //清除接受文件缓存
    RTC.prototype.cleanReceiveFile = function(sendId) {
        var that = this;
